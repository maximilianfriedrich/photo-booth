...
class Utils {
  ...
  convertImageForDownload(filename, grayscale, callback) {
	...
	var svgpath = this.getPhotosDirectory() + "watermark.svg";

	const TextToSVG = require('text-to-svg');
	const textToSVG = TextToSVG.loadSync();
 
	var attributes = {fill: 'white', stroke: 'black'};
	var options = {x: 0, y: 0, fontSize: this.getConfig().watermark.fontsize,kerning: true, anchor: 'top', attributes: attributes};
 
	const svg = textToSVG.getSVG(this.getConfig().watermark.text, options);
    ...
    if (grayscale) {
      sharp(_path) // resize image to given maxSize
        .grayscale()
		.overlayWith(svgpath, { gravity: sharp.gravity.southeast })
